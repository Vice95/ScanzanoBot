...
# @Last modified time: 2016-11-19T20:26:04+01:00
# @License: MIT License
...
class mainloop{
	...
	function shell($telegram,$text,$chat_id,$user_id,$location,$reply_to_msg,$nome,$cognome,$user,$image){
		...
		if ($text == "/start" || $text == "Informazioni") {
			...
			if ($text=="/start"){
				...
				if($this->cerca_utenti(URL_USER_CSV,$user_id)==0){
					$data  = array(	 'entry.59986005' => date("d/m/Y") ,
													 'entry.1082763602' => date("H:i:s") ,
													 'entry.1635869948' => $user_id ,
													 'entry.1376242742' => $nome ,
													 'entry.2097143125' => $cognome ,
													 'entry.663097702' => $user
												 );
					 $this->postData(URL_USER,$data);
				}
					$log=$today. ";new user started;@" .$user_id. ";  ".$nome."  ".$cognome."   ".$user."\n";
					$log.=$today. ";new chat started;" .$chat_id. "\n";
				}
			$this->create_keyboard_temp($telegram,$chat_id,"base",NULL,NULL);
			}
		else if ($text == "Foto") {
			$arrayfile=array();
		...
		/*************************Events*******************************/
			...
			$this->create_keyboard_temp($telegram,$chat_id,"eventi",NULL,NULL);
  	...
  	/*************************Trasporti*******************************/
			...
			$this->create_keyboard_temp($telegram,$chat_id,"trasporti",NULL,NULL);
		...
		/*************************POI*******************************/
			...
			$this->create_keyboard_temp($telegram,$chat_id,"luoghi",NULL,NULL);
		...
		/*************************News*******************************/
			...
			$this->create_keyboard_temp($telegram,$chat_id,"news",NULL,NULL);
		...
		/*************************Soldi Pubblici*******************************/
			...
			$this->create_keyboard_temp($telegram,$chat_id,"soldi",NULL,NULL);
		...
		$data  = array(	 'entry.1129081066' => $today ,
						         'entry.1342234530' => $text ,
						         'entry.1718319490' => $user_id ,
						         'entry.1552187523' => $chat_id ,
						         'entry.194242629' => $user);
		 $this->postData(URL_LOG,$data);
	}
	//cerca P.O.I.
	...
	function cerca_utenti($urlgd,$user_id){
				$inizio=1;
				$csv = array_map('str_getcsv',file($urlgd));
				//var_dump($csv);
				$count = 0;
  ...
  function load_rss($telegram,$user_id,$chat_id,$url){
			...
			foreach ($rss->item as $item):
				...
				if($i>5) break;
			endforeach;
			$chunks = str_split($news, self::MAX_LENGTH);
			foreach($chunks as $chunk) {
				$content = array('chat_id' => $chat_id, 'text' => $chunk,'parse_mode'=>'HTML','disable_web_page_preview'=>true);
				$telegram->sendMessage($content);
			}
			/*$content = array('chat_id' => $chat_id, 'text' => $news,'parse_mode'=>'HTML','disable_web_page_preview'=>true);
			$telegram->sendMessage($content);*/
		}
	function load_atom($telegram,$user_id,$chat_id,$url){
			$atom = Feed::loadAtom($url);

			$news="Ultime Notizie \n";
			$news.= htmlSpecialChars($atom->title)."\n" ;
			$news.= htmlSpecialChars($atom->description)."\n";
			$i=0;
			foreach ($atom->entry as $entry):
			  $news.="===========================\n";
				$news.="<b>".htmlSpecialChars($entry->title)."</b>\n";
				$news.="<i>".date("j.n.Y H:i", (int) $entry->timestamp)."</i>\n";
				$news.="".htmlspecialchars_decode($entry->link[4]['href'] )."\n";
				$i++;
				if($i>5) break;
			endforeach;
			$chunks = str_split($news, self::MAX_LENGTH);
	...
	function testata($telegram,$user_id,$chat_id,$url,$feed,$type){
		if($type=="atom"){
			$this->load_atom($telegram,$user_id,$chat_id,$feed);
		}
		elseif($type=="rss"){
			$this->load_rss($telegram,$user_id,$chat_id,$feed);
		}
	...
	function postData($formResponseurl,$data){
		//https://docs.google.com/forms/d/e/1FAIpQLSdXuN31mU-CbUtY86L4bTKsoiY5b6SS_B3nsO0acFOA1NAJRw/viewform?entry.1129081066=data&entry.1342234530=testo&entry.1718319490=userid&entry.1552187523=chatid&entry.194242629=user
